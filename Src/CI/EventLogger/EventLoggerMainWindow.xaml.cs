...
namespace EventLogger
{
  public partial class EventLoggerMainWindow : Window
  {
    ...
    async void onMark(object z, RoutedEventArgs e) { var s = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss}  Mark \n"; tbkBig.Text += s; await File.AppendAllTextAsync(App.TextLog, s); }
    async void onExit(object s, RoutedEventArgs e) { await File.AppendAllTextAsync(App.TextLog, $"{DateTime.Now:yyyy-MM-dd HH:mm:ss}  Closd manually\n"); Close(); }
    protected override async void OnClosed(EventArgs e) /**/   { await File.AppendAllTextAsync(App.TextLog, $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} {(DateTime.Now - App.Started):hh\\:mm\\:ss}  OnClosed  () \n"); base.OnClosed(e); SystemSounds.Hand.Play(); }
  }
}
