...
namespace AsyncSocketTester
{
    public partial class TesterMainWindow : WindowBase
    {
        ...
        void onRealReal(object s, RoutedEventArgs e) { var c = new AsynchronousClient(); c.ConnectSendClose("mtuatwebts01.bbssecurities.com", 6756, "alex.pigida"); tbkReportClt.Text += c.Report; }
        ...
        void onTag(object s, RoutedEventArgs e)
        {
            var c = new AsynchronousClient();
            var ip_p = ((Button)s).Tag?.ToString()?.Split(':') ?? new[] { "www", "123" };
            var ip = ip_p[0];
            var po = int.Parse(ip_p[1]);

            c.ConnectSendClose(ip, po, "alex.pigida"); 
